...
4. **节点命名规范（极其重要）**:
   ...
   - 节点显示文本必须用双引号包裹在方括号内: \`A["中文文本"]\`
   - **绝对禁止**: 在节点ID中使用中文、空格、特殊字符、标点符号
   - **绝对禁止**: 使用裸露的中文作为节点ID，如 \`开篇 --> 结局\`（错误）
   - **正确示例**: \`A["开篇"] --> B["结局"]\`
5. **中文标签**: 节点显示文本使用中文，确保可读性，但必须用双引号包裹
6. **边标签规范**: 边上的标签也必须用双引号包裹: \`A -->|"关系"| B\`
7. **特殊字符处理**: 
   - 避免在文本中使用可能破坏语法的字符：( ) [ ] { } | < > # & @ 等
...
### **Mermaid 代码示例** (使用分号分隔):
\`\`\`
graph TD;A["开篇：引入背景"] --> B["冲突：矛盾爆发"];A --> C["人物登场：主角出现"];B --> D["发展：情节推进"];D --> E["高潮：矛盾顶点"];E --> F["结局：矛盾化解"];subgraph 副线;G["副线开始"] --> H["副线发展"];end;D --> G
\`\`\`

\`\`\`
flowchart LR;subgraph 主角阵营;A["主角"] --> B["挚友"];A --> C["导师"];end;subgraph 反派阵营;D["反派"] --> E["手下"];end;A -->|"对抗"| D;B -->|"支援"| A;C -->|"指导"| A
\`\`\`
...
\`\`\`json
{
  ...
  "mermaid_diagrams": [
    ...
    },
    {
      "type": "graph",
      "title": "人物关系图",
      "code": "graph LR;A["主角"] --> B["配角A"];A --> C["配角B"];B --> D["反派"];C --> D"
    }
  ]
}
\`\`\`

- **overallAssessment**: 一个字符串。请对作品进行深度综合评价，不仅要总结当前的总体水平，还需结合市场趋势或文学价值预测其发展潜力。评价应包含对作品核心竞争力（如文笔、创意、人设）的定位以及目标受众分析 (string)。
...
- **summary**: 一个字符串。请提供**高信息密度**的作品概述。要求：1. **全景细节**：必须完整覆盖故事的起因、经过、高潮和结局（或当前章节的断点），不能遗漏关键转折点。2. **语言精炼**：去除所有冗余修饰词，使用干练的语言概括核心情节和深层主题。3. **重点突出**：在陈述事实的同时，点出作品独特的风格印记或核心矛盾 (string)。
- **tags**: 请为以下文本生成一个字符串数组形式的标签列表。标签应包含5-12个关键词。1. **优先匹配**：若涉及特定动漫/游戏/小说（如BangDream, MyGO, 原神等），必须优先列出。2. **多维覆盖**：包含题材（如“赛博朋克”）、风格（如“慢热”、“意识流”）及特殊元素（如“性转”、“群像剧”）。(string[])
- **dimensions**: 必须是一个包含全部16个维度的数组。每个对象必须包含 \`name\` (维度名称), \`score\` (该维度的得分或权重值), \`description\` (一句**基于证据**的精炼评分理由。请引用文本中的具体情节、对话或描写作为依据，解释为何给出该分数，避免空泛的评价) (string)。
