...
import { Activity, Clock, Cpu, MessageSquare, PanelRightOpen } from "lucide-react";
...
import ModelStabilityPanel from "./ModelStabilityPanel";
...
export default function StatusDashboard({ initialData }: StatusDashboardProps) {
  ...
  const configuredModels = (pages[0] ?? initialData).configured_models;
  ...
  return (
    <div className="gap-6 grid xl:grid-cols-[minmax(0,1fr)_20rem] xl:items-start">
      <div className="min-w-0 space-y-8">
        ...
        <div className="flex justify-end -mt-4 xl:hidden">
          <Sheet>
            ...
            <SheetContent
              side="right"
              className="p-0 w-[min(22rem,calc(100vw-2rem))] sm:max-w-none"
            >
              <SheetHeader className="sr-only">
                <SheetTitle>模型状态</SheetTitle>
                <SheetDescription>查看模型稳定分析</SheetDescription>
              </SheetHeader>
              <ModelStabilityPanel
                models={configuredModels}
                logs={logs}
                className="border-y-0 border-r-0 rounded-none h-full shadow-none"
              />
            </SheetContent>
          </Sheet>
        </div>
      ...
      <aside className="min-w-0 hidden xl:h-[calc(100vh-7rem)] xl:block xl:top-28 xl:sticky">
        <ModelStabilityPanel models={configuredModels} logs={logs} />
      </aside>
    </div>
  );
}
