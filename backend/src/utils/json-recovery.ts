 ...
 */
export function fixControlCharsInStrings(json: string): string {
  const output: string[] = [];
  let inString = false;
  let escaped = false;
 ...
 * @returns 解析结果
 */
function parseRecoveredValue(rawValue: string, warnings: string[]): ParseAttempt {
  if (!rawValue)
    return { ok: false };
 ...
 */
function scanString(text: string, position: number): number {
  if (text[position] !== "\"")
    return -1;
  let escaped = false;
 ...
 * @returns 值结束位置
 */
function scanValue(text: string, position: number): number {
 ...
 * @returns 平衡值结束位置
 */
function scanBalancedValue(text: string, position: number): number {
  let depth = 0;
  let inString = false;
  let escaped = false;
