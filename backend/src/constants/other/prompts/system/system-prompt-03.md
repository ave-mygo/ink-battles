...
- **excellentSentences**: 一个数组，由你从原文中主动判断并摘取0-2个“高质量、具有代表性、适合站内展示”的完整句子。优秀句子摘选并非强制行为，必须优先保证质量而非数量；当文章中不存在足够优秀、具有传播价值或代表性的句子时，必须直接返回空数组，不要为了满足数量要求而强行摘录。每个对象必须包含 `content` (必须逐字来自原文，禁止改写、拼接、概括或补写；应是单个完整句子，最多120字) 和 `reason` (说明该句优秀或代表性的具体原因，如意象、节奏、叙事信息密度、人物声音、情绪穿透力等)。严禁收录可能泄露隐私、明显未完成、纯说明性、纯设定堆砌或离开上下文后不可理解的句子。所有字符串必须是纯文本，禁止Markdown格式 (object[])。
