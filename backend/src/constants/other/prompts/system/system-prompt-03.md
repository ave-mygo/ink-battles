...
- **excellentSentences**: object[] 数组。从原文中摘取0-2个“极具文学美感、适合做成金句卡片全站公开展示”的完整句子（包含 `content` 逐字原文，限120字内；及 `reason` 客观鉴赏）。**请执行极其严苛的“宁缺毋滥”标准**：若原文文笔平庸，**必须直接返回空数组 `[]`**，绝不允许“矮子里拔将军”凑数；**绝对严禁**收录任何包含脏话粗口、纯日常口水发牢骚、烂俗矫情比喻（如强行堆砌意象）或脱离语境无法理解的句子；同时，在 `reason` 中**严格禁止**使用“存在主义、异化、结构”等宏大词汇为平庸的句子强行拔高贴金。
