 ...
 * @returns 队列最大长度
 */
const getQueueLimit = (pool: AnalysisTaskPool) =>
	pool === "sponsor" ? analysisConfig.max_sponsor_queued_tasks : analysisConfig.max_queued_tasks;
 ...
 * @returns 对应的队列数组
 */
const getQueue = (pool: AnalysisTaskPool) =>
	pool === "sponsor" ? sponsorQueuedAnalysisTasks : queuedAnalysisTasks;
 ...
 * @returns 是否可以接受新任务
 */
export const canAcceptAnalysisTask = (pool: AnalysisTaskPool) =>
	getQueue(pool).length + reservedAnalysisTaskSlotCounts[pool] < getQueueLimit(pool);
 ...
 * @returns 是否成功预留
 */
export const reserveAnalysisTaskSlot = (pool: AnalysisTaskPool) => {
 ...
 * @param pool - 任务池类型
 */
export const releaseAnalysisTaskSlot = (pool: AnalysisTaskPool) => {
 ...
 * @returns 是否成功加入队列
 */
export const runQueuedAnalysisTask = (
	taskId: ObjectId,
	options: AnalysisTaskOptions,
	executeTask: AnalysisTaskExecutor,
) => {
 ...
 * @param taskId - 任务 ID 字符串
 */
export const forgetAnalysisAbortController = (taskId: string) => {
	abortControllers.delete(taskId);
};
 ...
 * @param executeTask - 任务执行器函数
 */
const drainAnalysisQueue = (executeTask: AnalysisTaskExecutor) => {
