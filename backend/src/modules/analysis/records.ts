 ...
 * @returns 快照数据对象
 */
export function createTaskSnapshot(task: Record<string, any>) {
  return {
    success: true,
    status: task.status,
    error: task.error,
 ...
 */
export async function getAnalysisTaskResult(taskId: string) {
 ...
 * @returns 是否已过期
 */
function isGuestRecordExpired(record: Record<string, any> | null, now = Date.now()) {
 ...
 */
async function activateOrHideGuestRecord(record: Record<string, any>) {
  if (!record)
    return null;
 ...
 * @returns 可序列化的记录对象
 */
function createTaskResultRecord(record: Record<string, any>) {
  return {
    ...record,
