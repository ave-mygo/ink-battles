...
此部分调用次数是用户当月捐赠行为的额外回馈。采用“加法”保底机制，并设置**更高的虚拟单价**和**更低的保底次数**，以更好地平衡激励与成本，并引导付费。
...
```
每月赠送调用次数 = min( (每月赠送保底次数 + floor( 当月捐赠金额 / 赠送调用虚拟单价 ) ), 每月赠送上限 )
```

其中：

- `当月捐赠金额 (D_month)`：会员当月捐赠的人民币金额。
- `赠送调用虚拟单价`：人民币 1.20 元。
- `每月赠送保底次数`：10 次。
- `每月赠送上限`：80 次。
- `floor()` 函数：向下取整。
