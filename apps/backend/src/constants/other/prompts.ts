...
export const ANALYSIS_MODE_INSTRUCTIONS: Record<string, string> = {
  初窥门径: `
## 评分模式：✅ 初窥门径
...
export const STANDARD_MODE_INSTRUCTION = `
## 评分模式：⚙️ 标准模式
 ...
 */
export async function buildSystemPrompt(modeInstruction: string): Promise<string> {
 ...
 */
export async function getModeInstructions(mode: string | string[]): Promise<string> {
  ...
  return modes.map(item => ANALYSIS_MODE_INSTRUCTIONS[item] ?? STANDARD_MODE_INSTRUCTION).join("\n\n");
}
