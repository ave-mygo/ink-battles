 ...
 */
export async function queryAfdianSponsors(page: number) {
 ...
 */
export async function queryAfdianOrder(orderNo: string) {
  const { afdian } = getConfig();
 ...
 */
export async function verifyOrderOwnership(orderNo: string, afdId: string) {
 ...
 */
export async function exchangeAfdianCode(code: string, redirectUri = getConfig().afdian.redirect_uri) {
  const { afdian } = getConfig();
