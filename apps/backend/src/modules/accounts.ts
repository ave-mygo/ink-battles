...
import { consumeEmailCode, sendEmailCode } from "../utils/email-codes";
 ...
 * 发送邮箱绑定验证码
 * @param request - HTTP 请求对象，包含用户认证信息
 * @param body - 请求体，包含待绑定邮箱地址
 * @returns 操作结果，包含成功状态和提示消息
 */
  .post("/api/v2/rpc/accounts.sendEmailBindingCode", async ({ request, body }) => {
    await requireUser(request.headers);
    return sendEmailCode(normalizeEmail(body.email), "register");
  }, { body: t.Object({ email: t.String() }), detail: { tags: ["RPC: Accounts"] } })
/**
 * 绑定邮箱账号
