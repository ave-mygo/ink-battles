 ...
 */
export function normalizeVerificationType(value?: string): VerificationType {
  return value === "login" || value === "reset-password" ? value : "register";
}
 ...
 */
export async function consumeEmailCode(email: string, code: string, type: VerificationType) {
  const now = new Date();
