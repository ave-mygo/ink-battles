...
import { Monitor, ShieldAlert, ShieldCheck, Smartphone, X } from "lucide-react"
...
function SessionRow({ session, disabled, onRevoke }: { session: SessionInfo; disabled?: boolean; onRevoke: () => void }) {
  return (
    <div className="flex flex-col gap-3 rounded-sm border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="flex min-w-0 flex-col gap-1">
          <span className="truncate text-[13px] font-medium text-zinc-900 dark:text-zinc-100" title={session.userAgent || "未知设备"}>
      ...
      <div className="flex shrink-0 justify-end sm:min-w-24">
        {session.current ? (
          <span className="inline-flex h-8 items-center gap-1.5 rounded-sm border border-emerald-200 bg-emerald-50 px-3 text-[11px] font-medium text-emerald-700 dark:border-emerald-900/60 dark:bg-emerald-950/20 dark:text-emerald-300">
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            className="h-8 border-red-200 px-3 text-[11px] text-red-500 hover:border-red-300 hover:bg-red-50 hover:text-red-600 dark:border-red-950/70 dark:text-red-400 dark:hover:bg-red-950/30"
            disabled={disabled}
            onClick={onRevoke}
          >
